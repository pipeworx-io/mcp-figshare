...
`tools/list` at `https://gateway.pipeworx.io/figshare/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/articles \
  -H 'Content-Type: application/json' \
  -d '{"query":"climate change datasets"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/articles`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
